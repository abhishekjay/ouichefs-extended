...
/* The new IOCTL command you added in Task 1.10 */
...
/* Send the magical custom defrag command! */
if (ioctl(fd, OUICHEFS_IOC_DEFRAG_FILE) < 0) {
	perror("Defrag IOCTL failed");
} else {
	printf("Defrag complete! Run your test_ioctl tool again to see the new layout.\n");
}
